...
abstract class Zend_Db_Adapter_Abstract
{
    ...
    public function insertMultipleRecords($table, array $data)
    {
        // extract and quote col names from the array keys
       $valss='';
    ...
    public function updateMultipleRecords($table, array $dataUpdate,array $where) {
        //print_r($dataUpdate);die;
        $tableName = $table;
        $j = 0;
        foreach ($dataUpdate as $dataUpdateTemp) {
            $querySet = "Update ".$tableName." set ";
            foreach($dataUpdateTemp as $key=>$colum) {
                $querySet .= $key." ='".$colum."' ,"; 
            }
            $querySet = rtrim($querySet,",");
            $querySet .= ' where ';
            foreach ($where as $whereTemp) {
                $querySet .= $whereTemp.'="'.$dataUpdateTemp[$whereTemp].'" and ';
            }
            $querySet = chop($querySet,"and ");
            $query[$j] = $querySet;
            $j++;
        }
